...
import ch.minova.core.xml.tables.TableDocument;
...
import ch.minova.core.xml.tables.TableDocument.Table.Primarykey;
import ch.minova.core.xml.tables.TableDocument.Table.Uniquekey;
import ch.minova.core.xml.tables.TableDocument.Table.Values;
...
public class XmlDatabaseTable {
	...
	private void read(final InputStream is) {
		try {
			final TableDocument td = TableDocument.Factory.parse(is);
			final Table t = td.getTable();

			// der angegeben Name aus der xml-Datei wird hier in "name"
			// geschrieben
			this.name = t.getName();
			final Column[] cols = t.getColumnArray();
			for (int i = 0; i < cols.length; i++) {
				final XmlDatabaseColumn dbc = new XmlDatabaseColumn(t, cols[i], this.DBCollation);
				this.columnTable.put(dbc.getName(), dbc);
				if (cols[i].getName().equalsIgnoreCase("Keylong")) {
					// nicht immer auf identitiy setzen
					if (cols[i].getInteger().getIdentity() == true) {
						setIdentity(true);
					}
				}
			...
			if (t.getPrimarykey() != null) {
				...
				final String[] colNames = primaryKey.getColumnArray();
				for (int i = 0; i < colNames.length; i++) {
					this.primaryKeyConstraint.addColumnName(colNames[i]);
				}
			}
			final Foreignkey[] fkeys = t.getForeignkeyArray();
			for (int i = 0; i < fkeys.length; i++) {
				final XmlForeignKeyContraint fkc = new XmlForeignKeyContraint(t, fkeys[i], fkeys[i].getColumnArray());
				this.foreignKeyContraint.add(fkc);
			}
			final Uniquekey[] ukeys = t.getUniquekeyArray();
			for (int i = 0; i < ukeys.length; i++) {
				final XmlUniqueKeyConstraint ukc = new XmlUniqueKeyConstraint(t, ukeys[i], ukeys[i].getColumnArray());
			...
			if (v != null) {
				final XmlValues xmlv = new XmlValues(v.getColumnArray(), this.name);
				setXmlvalues(xmlv);
				for (int i = 0; i < v.getRowArray().length; i++) {
					this.xmlvalues.addValueRow(v.getRowArray(i).getValueArray());
				}
			}

			final TableDocument tdout = TableDocument.Factory.newInstance();
			tdout.addNewTable().setName("Test");
			tdout.getTable().addNewColumn().setName("KeyLong");
	...
	public void saveXML(StringWriter sw) {
		final TableDocument td = TableDocument.Factory.newInstance();
		td.addNewTable();
		td.getTable().setName(this.name);
		final Table t = td.getTable();
