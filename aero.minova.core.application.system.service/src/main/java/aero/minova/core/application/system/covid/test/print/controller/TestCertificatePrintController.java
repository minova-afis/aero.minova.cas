...
public class TestCertificatePrintController {
	...
	public @ResponseBody byte[] getTestCertificate(Integer keyLong) throws Exception {
	...
	private String testCertificateReportXml(Integer keyLong) {
		...
		try {
			...
			reportBody = t.getResultSet().getRows().get(0).getValues().get(0).getStringValue();
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		return "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>" + "<CTSTestZertifikat>" + reportBody + "</CTSTestZertifikat>";
	}
}
