...
Die XTable ist eigentlich dasselbe wie eine normale Table mit dem Unterschied,
 dass sie noch eine Id beinhaltet. Diese ist wird immer als String behandelt.
[source,json]
--------
{
	"id" : "IdName",
...
Führt eine List an voneinander abhängenden Prozeduren auf dem Server aus.
Hierbei wird dieselbe Funktionalität wie bei data/procedure verwendet.
Zusätzlich werden jeodch vor dem Ausführen der einzelnen Prozeduren 
die Platzhalter-Values durch die referenzierten Values aus den Rückgabe-Werten ersetzt.

=== Eingabe

Als Eingabe wird eine Liste an xTables im Body der Get-Anfrage erwartet.

Jede xTable enthält eine Id, die Parameter für die SQL-Prozedur und optional Metadaten.
Die columns beschreiben den Namen und den Typ der Argumente.
Die rows enthalten eine Zeile, in der die Argumente gelistet sind.

=== Ausgabe

Die Antwort ist eine Liste an xSqlProcedureResults und hat bei Erfolg folgendes Format:
...
==== resultSet, outputParameters, returnCode

Das ResultSet, die outputParameters und das Verhalten des ReturnCodes ist dasselbe wir beim Ausführen von <<CAS API,data/procedure>>.
