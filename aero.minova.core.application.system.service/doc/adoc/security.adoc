...
seinen GruppenSecurityTokens(Memberships) ist in der Tabelle tUser hinterlegt.
...
Falls kein Eintrag für den eingeloggten User in der Tabelle tUser befindet, gilt sein Username als einer der SecurityTokens.
...
Bei jedem Aufruf der Methode _getIndexView(Table_ _inputTable)_ wird auf der View vCASUserPrivileges nach den nötigen Berechtigungen gesucht.
Dies geschieht mit der Methode _checkPrivilege(Table_ _inputTable)_ in folgender Form:

[source, sql]
----
select PrivilegeKeyText,KeyText,RowLevelSecurity from vCASUserPrivileges
...
Hierfür brauchen die betroffenen Tabellen natürlich eine Spalte '_SecurityToken_', in welcher diese abgefragt werden können.
...
Die SecurityToken werden dann mithilfe der Tabelle tColumnSecurity abgefragt, ob einer dieser Tokens dazu berechtigt ist, die gesamte Tabelle zu sehen.
