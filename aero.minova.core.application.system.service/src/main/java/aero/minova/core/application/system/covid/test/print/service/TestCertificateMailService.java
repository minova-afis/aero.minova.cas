...
public class TestCertificateMailService {
	...
	public void sendCertificate() throws Throwable {
		...
		sqlViewController.unsecurelyGetIndexView(sqlRequest, Arrays.asList(requestingAuthority)).getRows().stream()
				.map(row -> row.getValues().get(0).getIntegerValue()).forEach(terminKey -> {
	...
	public List<String> targetAddresses(Integer testTerminKeyLong) {
		...
		final String reportBody;
		try {
			val t = sqlProcedureController.calculateSqlProcedureResult(sqlRequest);
			val rawAddressValues = t.getResultSet().getRows().stream().map(row -> row.getValues().get(0).getStringValue()).filter(e -> e != null)
					.collect(toList());

			t.getResultSet().getRows().stream().map(row -> row.getValues().get(1).getStringValue()).filter(e -> e != null)
					.forEach(e -> rawAddressValues.add(e));
			val rawAddresses = rawAddressValues.stream().reduce("", (a, b) -> a + "; " + b).split(";");
			return Arrays.asList(rawAddresses).stream().filter(e -> e != null).map(e -> e.trim()).filter(e -> !e.isEmpty()).distinct().collect(toList());
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
