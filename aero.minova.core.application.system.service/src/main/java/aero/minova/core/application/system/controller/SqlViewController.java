...
public class SqlViewController {
     ...
     *
     * @param securityTokens Die Gruppen, die dem anfordenden gehören.
    ...
    public Table columnSecurity(Table inputTable, List<Row> userGroups) {
        ...
        for (Row row : userGroups) {
            if (row.getValues().get(0).getStringValue().equals(inputTable.getName())) {
                Row bar = new Row();
                bar.setValues(
                        asList(new Value(inputTable.getName(), null), new Value("", null), new Value(row.getValues().get(1).getStringValue(), null)));
