...
class JOOQViewServiceTest extends ViewServiceBaseTest<JOOQViewService> {
	...
	void testPrepareViewString_withSelectByMultipleAttributeValue() {
		...
		assertThat(testSubject.prepareViewString(inputTable, true, 1000, userGroups))//
				.isEqualTo(
						"select EmployeeText, CustomerText from vWorkingTimeIndex2 where (cast(EmployeeText as varchar) ilike ? and cast(CustomerText as varchar) ilike ?)\r\norder by EmployeeText, CustomerText limit ?");
	}
	...
	void testPrepareViewString_withSelectByMultipleConditionsOnSameAttribute() {
		...
		assertThat(testSubject.prepareViewString(inputTable, true, 1000, userGroups))//
				.isEqualTo(
						"select BookingDate from vWorkingTimeIndex2 where (BookingDate <= ? or BookingDate < ? or BookingDate >= ? or BookingDate > ? or BookingDate <> ?)\r\norder by BookingDate limit ?");
	}
	...
	void testPrepareViewString_withSelectByMultipleOptionalRules() {
		...
		assertThat(testSubject.prepareViewString(inputTable, true, 1000, userGroups))//
				.isEqualTo(
						"select EmployeeText from vWorkingTimeIndex2 where (cast(EmployeeText as varchar) ilike ? or cast(EmployeeText as varchar) ilike ?)\r\norder by EmployeeText limit ?");
	}
	...
	void testPrepareViewString_withSelectByAttributesWithoutConstraint() {
		...
		assertThat(testSubject.prepareViewString(inputTable, true, 1000, userGroups))//
				.isEqualTo(
						"select EmployeeText, CustomerText from vWorkingTimeIndex2 where cast(EmployeeText as varchar) ilike ?\r\norder by EmployeeText, CustomerText limit ?");
	}
