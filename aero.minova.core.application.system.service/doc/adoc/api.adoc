...
Generell gilt, dass jedes Fled bis auf metaData ein Pflichtfeld ist.
...
Die rows enthält eine Zeile in der die Argumente gelistet sind.
...
Diese sucht anhand der Ticket Nummer, die Daten des Tickets im Trac
und im SQL-Server nach.

[source,json]
.Als Parameter muss die `TicketNumber` übergeben werden, wobei das Ticket -123 ein nicht existierende Test-Ticket ist, welches immer abrufbar ist:
--------
{
    "name": "Ticket",
    "columns": [
