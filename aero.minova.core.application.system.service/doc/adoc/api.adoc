...
Basierend auf limited wird die n-te Seite der totalPages ausgegeben.
Ist kein Wert für page gesetzt, werden die Ergebnisse von 0 bis limited,
also die 1.Seite, zurückgegeben.

==== metaData/totalResults

Zeigt an, wie viele Rows für die angegebenen Suchparameter gefunden wurden.

==== metaData/totalPages

Gibt an, wie viele Seiten basierend auf den totalResults und limited errechnet wurden.

==== metaData/resultsLeft

Gibt an, wie viele Rows nach der jetzigen Seite noch übrig sind.
