...
Falls keine KeyLong-Spalte in dem ResultSet gefunden werden kann, wird auf den KeyLong der ersten ausgeführten Prozedur (Haupt-Prozedur) der Anfrage zurück gegriffen.
Wenn es auch dort keinen KeyLong gibt (oder es gar kein ResultSet gibt) wird der KeyLong aus der Input-Tabelle der Haupt-Prozedur verwendet.
Wird gar kein passender Parameter gefunden wird ein entsprechender Fehler geloggt.
