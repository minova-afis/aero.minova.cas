...
[TIP]
====
Es ist zu überlegen, ob man bei den Werten auch mehrere `rows` übergeben darf. 
Dies würde dann mehreren Aufrufen der Prozedur mit den Werten entsprechen.

Die Frage ist dann, wie die Rückgabewerte für einen Aufruf identifiziert werden.
Man könnte sich den Array-Index-Operator (`[]`) verwenden.
====
